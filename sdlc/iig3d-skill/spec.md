...
routing: {linear-progression: {primary: 3d-disc-timeline, alternates: [3d-arrow-ribbon, 3d-slab-stack, 3d-gradient-pedestal, 3d-cylinder-column]}, ...22 rows}
```
...
- `tests/test_routing.py`: R4 — parametrised over all 22 routing rows plus the 12 device names and explicit member styles; unknown names exit 1.
...
- `tests/test_cli.py`: R16, R11 — every subcommand prints exactly one JSON object on stdout; `list` counts 12 members and 22 layouts.
