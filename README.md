...
the selector never passes two watermarked references together. Several shipped user-added
refs carry the flag (stock previews); `check --allow-watermark` is the clean baseline, and a
pinned watermarked ref renders with a warning. Rights for images you add stay
with you.
...
skills/iig3d/
  ...
  refs/<member>/ref-NN-*.jpg  # reference images; user-added ones may carry the watermark flag
